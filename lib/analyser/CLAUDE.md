...
- **A downbeat tracker exists offline, and the lesson it left here is about tempo versus phase.** The octave fold documented above deliberately does not touch beat phase, so a front end can report the right *number* for the tempo while its *grid* sits a fraction of a beat off the music, and anything downstream that needs bars rather than beats inherits that. It is why the bar-phase decoder admits half-beat positions as states rather than rounding them away. The tracker's own v1 scoring ran on the aubio beat stream and was removed when madmom replaced it (owner decisions #81/#133); `training/nn/CLAUDE.md` carries the note and `docs/migration-evidence.md` the successor numbers.
