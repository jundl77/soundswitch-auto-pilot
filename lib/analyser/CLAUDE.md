...
- **DROP first** because it is the only branch requiring positive evidence from two independent features (density *and* a beat-locked kick). Nothing weaker should be able to pre-empt it.
...
- **Kick strength across a corpus**: the presence threshold sits in a wide empty band on the reference track. Confirm that band exists on other material, and tune the kick-absence margin against passages where the kick drops out mid-groove.
