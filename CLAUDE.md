...
- **Cleanliness is a gate, not a cleanup.** A track is admitted only if ffmpeg decodes the whole file without a single error line *and* the container duration agrees with the annotation record's duration. Both checks are needed and neither implies the other: a truncated mp3 keeps the full length advertised in its Xing header, so it passes a duration check while failing to decode; and a perfectly readable file can still be the wrong video, in which case every beat-to-label join drifts. Rejected tracks are recorded with their reason rather than deleted -- a wrong-length track is usually a wrong-video match worth a human's eye, not a bad byte stream. The tolerance is deliberately loose (an absolute floor for normal tracks, proportional for long DJ edits) because it exists to catch a different recording, not encoder padding.
