...
- **A 403 on the media URL is our problem, not YouTube's policy.** It means a signature/nsig challenge could not be solved, which is a property of the toolchain, not of the video. It gets its own reason bucket rather than folding into `bot_check` because the two demand opposite remedies, and reporting one as the other sends the owner after credentials they do not need. Two independent causes, both seen on the first corpus sweep, and the second is the one that bites: **(1)** a JS runtime that is installed but not *visible to the running process* — a shell started before the install carries a stale PATH, so "installed" must be checked from the runner, not a fresh terminal; **(2)** yt-dlp's remote challenge-solver components are **opt-in** and were never enabled — 43 of the 62 403s happened while a runtime *was* visible, and yt-dlp says so itself in the failure records (`[jsc] Remote components challenge solver script (deno)… were skipped… enable with --remote-components ejs:github`). A visible runtime is necessary but not sufficient.
- **Prerequisites.** `yt-dlp` and `ffmpeg` on PATH, plus a JavaScript runtime (Deno or Node) *visible to the running process*, **and** `--remote-components ejs:github` so yt-dlp actually downloads the challenge-solver it needs. Without both, downloads fail with `HTTP Error 403: Forbidden`. Installed is not the same as visible: a shell or detached process started before the install inherits a stale environment block. Check from the actual runner, not from a fresh terminal.
- **The corpus data directory holds *ops copies* of the scripts, and they drift.** `raveform_download.py` is copied next to the corpus and `raveform_supervisor.py` lives *only* there (it is deliberately not in git — it is operational tooling for one incident, not application code). A supervised refresh runs those copies, not the branch, so **whenever the downloader changes, refresh `<data-dir>/raveform_download.py` from the branch**. This is not hypothetical: the ops copy carried the pre-`http_403` classifier and a supervisor hardcoded to `--retry-reasons bot_check`, which would have stranded every recoverable 403 on the next refresh while the branch looked correct. The supervisor now reads `RETRYABLE_REASONS` from the downloader sitting beside it rather than keeping its own copy of the list, so that particular drift cannot recur — but the file-copy step still has to be done by hand.
