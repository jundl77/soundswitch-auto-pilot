...
- **A 403 on the media URL is our problem, not YouTube's policy.** It means a signature/nsig challenge could not be solved, which is a property of the toolchain, not of the video. It gets its own reason bucket rather than folding into `bot_check` because the two demand opposite remedies, and reporting one as the other sends the owner after credentials they do not need. Two independent causes, both seen on the first corpus sweep, and the second is the one that bites: **(1)** a JS runtime that is installed but not *visible to the running process* — a shell started before the install carries a stale PATH, so "installed" must be checked from the runner, not a fresh terminal; **(2)** yt-dlp's remote challenge-solver components are **opt-in** and were never enabled — of the 68 recorded 403 events, 43 happened while a runtime *was* visible (the supervisor's own preflight found it and logged no warning), and yt-dlp says so itself in four of them (`[jsc] Remote components challenge solver script (deno)… were skipped… enable with --remote-components ejs:github`). A visible runtime is necessary but not sufficient.
- **Prerequisites.** `yt-dlp` and `ffmpeg` on PATH, plus a JavaScript runtime (Deno or Node) *visible to the running process*. Installed is not the same as visible: a shell or detached process started before the install inherits a stale environment block, so check from the actual runner, not from a fresh terminal. Without a visible runtime — and sometimes even with one, since yt-dlp's challenge-solver components are opt-in — downloads **can** fail with `HTTP Error 403: Forbidden`. Not *will*: the whole 1,387-track corpus was fetched without ever passing the flag, and the 403s that did occur were recoverable by re-attempting. `raveform_download.py --remote-components ejs:github` forwards the flag verbatim to yt-dlp when a 403 wave needs clearing; it stays off by default because it makes yt-dlp download and run code at fetch time, which is the owner's call.
