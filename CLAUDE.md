...
- **A 403 on the media URL is our problem, not YouTube's policy.** It means a signature/nsig extraction could not be solved, which is a property of the toolchain — usually a JS runtime that is installed but not visible to the running process, since a shell started before the install carries a stale PATH. It gets its own reason bucket rather than folding into `bot_check` because the two demand opposite remedies, and reporting one as the other sends the owner after credentials they do not need.
- **Prerequisites.** `yt-dlp` and `ffmpeg` on PATH, plus a JavaScript runtime (Deno or Node) *visible to the running process* — without one, yt-dlp cannot solve YouTube's signature challenges and downloads fail with `HTTP Error 403: Forbidden`. Installed is not the same as visible: a shell or detached process started before the install inherits a stale environment block and sees neither. Check from the actual runner, not from a fresh terminal.
