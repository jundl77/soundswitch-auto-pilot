...
- **Nothing here is runnable, and the gap is larger than an integration task.** Three things gate a live show, in descending order of size:
  1. **Live downbeat tracking is built offline and not wired in.** The decoder is bar-rate -- every decision, duration floor and boundary read is expressed in bars -- and offline it is handed an expert-annotated bar grid. `lib/` still has no downbeat tracker, and the training table records bar position as unknown for exactly this reason. What exists in `training/nn/` is the machinery: a downbeat head, a bar-phase decoder over a candidate grid, and an evaluation harness that scores a predicted bar grid against the annotator's on whatever beat stream it is handed.

     **The v1 scoring of that chain is removed rather than carried as a dated record.** It bound to the aubio beat stream, which madmom has since replaced, so its figures and its BLOCKED-at-0.85 verdict are superseded (owner decisions #81/#133) — the removal note is in `training/nn/CLAUDE.md` and the successor measurement is committed in `docs/migration-evidence.md`: gate-faithful downbeat F1 0.50 on val against 0.71 on the annotator's own grid, the 0.85 gate retired as sitting above published *offline* SOTA on general music, and F1 >= 0.55 at a median of two phase flips per track or fewer recommended in its place. The expert-grid bound is the figure that never depended on the beat source: it is what the head and the decoder are worth on a clean grid, and a better beat source moves the live number towards it without moving it.
...
- **Beat-absence ATMOSPHERIC is realigned one look-ahead late.** The engine now sends every intent commit through the delayed command queue, so a consumer can de-shift a block by recognising that its stamp minus the look-ahead lands on a beat. That test cannot recognise the one block a *timer* produced, so `realign_intents` leaves beat-absence ATMOSPHERIC where it is and the block sits a look-ahead late in song time. It costs nothing today: the only place it fires is after the audio runs out, past the last label, where it is scored against nothing — and mid-song it never fires at all (see ATMOSPHERIC never fires on mastered EDM). It becomes real the moment either of those two facts stops holding. `IntentAlignment.song_stamped` in `training/build_training_table.py` is the counter to watch.
- **The eval-set baseline lags a deliberate pipeline change by one command.** Any change to `lib/` or `simulate/` that moves the reports fails `run_eval_set.py` until the baseline is re-cut. That is the gate working, not a flake — but it does mean a pipeline PR is two steps, and the second one must not be skipped. The madmom migration is the worked example: every one of the ten checksums moved, and the baseline was re-cut in the merge that took the new beat source. It then happened a second time, in the merge that took master's post-review simplification — which is the more instructive case, because that merge changed no rhythm at all and still moved all ten checksums. **The gate keys on report content, not on musical behaviour**, so a bounded timing log or any other report-shaped change re-cuts it. Read the printed table before assuming a checksum move means the show moved: `beats` and `changes_intent` sitting still is the signal that it did not.
