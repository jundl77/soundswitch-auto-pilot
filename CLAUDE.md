...
The gitignored corpus does not follow `git worktree add`, so a linked worktree finds the main checkout's copy (`training/corpus_root.py` owns that resolution, and it is stdlib-only so a *show* can ask it without pulling the benchmark harness into production startup); `$RAVEFORM_DATA_DIR` overrides. **The existence of the corpus directory is not evidence that it is the corpus, and reading it as such made every worktree quietly weaker.** Because the hand labels are committed, `git worktree add` materialises `training/data/raveform` holding nothing but them — so the old existence test found it, stopped there, and never fell back. Measured, not hypothetical: a full `uv run pytest` in a worktree skipped the three artifact-gated simulation tests and reported success, and the `nn_artifacts` skip line made that look legitimate; live, the show itself resolved no model and ran the degradation state on a machine that had one. The probe is therefore for what only a machine's own copy has — the models dir the show reads, or the manifest acquisition writes — never for the directory.
...
2. Extracts rhythm via madmom's online neural trackers (beats, BPM). The beat stream drives OS2L, the overlay chase and the live bar grid; a fine-tuned Beat This downbeat tracker over trailing windows supplies per-beat bar-phase evidence, fused with the beat stream in a cyclic bar-phase HMM (the counting rule is the fallback when the tracker is absent or shed)
3. Resamples the same audio to the encoder's rate and runs a MERT encoder plus an online student over it, emitting one class posterior and one boundary score per label cell, in the full raw Raveform vocabulary (`intro / altintro / buildup / breakdown / bridge / drop / cooldown / outro / altoutro`)
4. Commits one immutable decision per bar through a fixed-lag Viterbi decoder, and maps that class to a `LightIntent` (ATMOSPHERIC / BREAKDOWN / BUILDUP / DROP)
  ...
  PyAudio --> MusicAnalyser (madmom online) --> beats, BPM, silence gate
         \
          -> resample + ring write ==================> MERT encoder
                                                       online student (ONNX)
                                                       bar tracker (Beat This)
          <== bounded hand-off queue (whole passes) <==/
          |
          BarPhaseFusion (beats + downbeat evidence -> committed bar phase)
          SectionDecoder  (fused bar grid  +  fixed-lag Viterbi)
                |
...
| `lib/label_space.py` | The one label vocabulary â€” the raw Raveform nine, the `end` drop, and the class-space check every priors file, graph and config is held to |
...
| `lib/analyser/bar_tracker.py` | The live bar tracker: the fine-tuned Beat This-small over trailing 30 s windows, one forward per stride, newest margin-safe downbeat logits out; artifact + sha + geometry verification at construction; a dead tracker is the counting grid, never a shed |
| `lib/engine/bar_phase.py` | `BarPhaseFusion` -- tracker chunks aggregated onto beat instants, gated into rotated position evidence, decided by the cyclic fixed-lag bar-phase HMM with forward-only corrections; base-plus-offset, so no evidence IS the counting grid |
| `lib/vendor/beat_this_infer.py` | Vendored Beat This inference path (MIT, attribution beside it) -- model, log-mel frontend, checkpoint load -- rewritten without einops/rotary-embedding-torch/torchaudio and numerically verified against the original on the shipped checkpoint |
...
| `lib/engine/light_engine.py` | `LightEngine` â€” decoder decisions in, a show out; owns the boundary refresh, the cold-start floor and the per-stream delay |
...
| `simulate/tracker_cache.py` | The bar tracker's chunks, cached beside the audio on the same pattern -- keyed on checkpoint sha + framing + audio identity; a warm run replays both stages or neither |
...
| `training/module_source.py` | Which checkout an offline script's `training.nn` came from, asserted rather than assumed |
...
| `training/label_tool.py` | `auto_pilot label` â€” a Dash app over one track's waveform, onset curve and beat grid, where the owner cuts a track into sections in the raw Raveform vocabulary plus a per-boundary strength. A labelling has two homes and the split *is* the lifecycle: working state is scratch CSV under `<corpus>/tmp_labels/` -- resolved through `corpus_root` exactly as a committed label is, because two resolutions meant a relaunch from another worktree silently showed an empty labelling while the work sat elsewhere -- rewritten on every edit so the file is the state and the page only a view of it. **Both halves of "only a view" are load-bearing and neither was true at first**: the layout is a function, so every page load re-reads the file, and an edit arriving from a page whose sections disagree with the file is refused rather than applied. A layout built once served launch-time sections to every reload, and since that stale list is the State each edit is applied to, one edit after a reload rewrote the file as launch-time state plus that edit -- silently destroying every boundary made in between. The refusal covers what the re-read cannot: a second tab. **A refusal nobody sees is the same as silence**: each launch mints a token baked into the page, a mutation from a tab of an earlier launch (the per-song relaunch leaves them looking alive) is refused by token before the file is read, a polled banner tells that tab it no longer controls a song and disables commit, and every refusal or success renders in a coloured status strip beside the commit controls -- validation logic unchanged, only made visible. A working file the tool cannot parse -- a time that is not a number, a label outside the vocabulary -- is refused by name at launch, before the decode, rather than raised as a traceback out of the pre-app path; and no edit may overwrite a file that could not be read, because that file is the only copy of whatever is being repaired. **`--seed` starts a labelling from the published boundaries rather than from nothing**, because relabelling is measurably a re-*naming* job: on the one benchmark track the owner redid, every one of his section starts was byte-exact the published start and 57 % of the names changed. The seed is a draft and says so — the working format carries no gaps, so an unlabelled published span (the `end` sentinel included) inherits its predecessor's name — and it never writes over an existing working file, which is the only copy of whatever is already labelled. The beat grid under it is the expert one where the corpus has it: published grids are filed under a corpus key rather than the track id and number the beat within its bar, where a generated grid flags the downbeat, so they are found by a second glob and read through that difference. The title and genre inputs open pre-filled with whatever the dataset already knows about these bytes — an existing hand label, a third-party source annotation, or the published record, in that order, falling back to the filename — so the owner corrects a prefill instead of retyping; the commit gate itself is unchanged. The page collects artist and track title as two pre-filled fields and composes the dataset's stored `Artist - Track` title itself — typing the separator was the owner's job and failed on a unicode dash — and every press stamps the status line, so a repeated identical refusal never reads as nothing happening. **Commit** promotes it into the dataset as `annotations/<track_id>.hand.json`, copying the audio into the corpus if it is not already there, and hands off to the corpus admission step if this branch has one. `segments.json` is never written to -- the benchmark hashes it. Places files only; git is the owner's action. Reads nothing from `lib/` or `simulate/`; the CLI reaches *it* the way the show reaches `corpus_root`, which is what keeps dash off the show's import path |
...
Four intents map to structural moments in an EDM track, and the mapping is
**many-to-one**: nine classes, four looks.
...
nearest existing look so the show stays runnable; the real assignment is a
product decision to be taken against measured per-class energy and position
profiles rather than intuition. `SECTION_CLASS_INTENTS` in
`lib/engine/effect_definitions.py` is the one dict that changes when it is.
**The profiles are now cut** — `<corpus>/models/l9_campaign/energy_profiles.md`
(plus `.json`, from the 1.2M-beat 9-class table) — and two readings in them are
worth the owner's eye: within each atmospheric family the `alt*` variant is the
audibly quieter one (so an alt split is decidable from energy alone, without
the position no window can see), and `cooldown` is measurably *louder* than
`breakdown` while sitting late in the track, so the placeholder currently plays
a quiet pool over near-drop energy.
...
priors' classes and the config's recorded space must all equal `SECTION_LABELS`
exactly — the subset tolerance that let a 5-class chain construct survives only
in the offline decoder, where replaying an older generation is legitimate. The
benchmark's gated space is the raw nine, and its
...
the probe reads, the per-arm results and the pre-registered arm ladder — and
...
buffer -- each of them fitted to the corpus rather than chosen. Nothing of it
remains: with PEAK removed the engine derives no show state of its own at all.

**The bar grid is tracked now, and counting is its degradation state** (#332,
the adoption of the #322-#331 bar-tracking campaign). A fine-tuned Beat
This-small runs on the GPU thread beside the MERT pass: every stride of new
audio it re-reads the trailing 30 s window and emits the newest margin-safe
downbeat activations -- a non-causal net over a trailing window is causal at
session level, and its multi-second evidence lag is exactly what the
forward-only-correction contract absorbs. The activations are aggregated onto
madmom beat instants, gated, and fed as rotated position evidence into the
cyclic bar-phase HMM from `training/phase_tracking/` (four positions, the
interval-ratio slip process, a fixed-lag decode, forward-only corrections).
**The net never emits the grid**: the HMM's committed count is base-plus-offset
where the base is the plain beat count, so with no evidence -- tracker absent,
tracker dead, or the stage shed -- the offset freezes and the grid IS today's
counting rule, and the measured start prior itself reproduces the counting
anchor. `tests/test_bar_phase_equivalence.py` holds the online fusion to the
campaign's offline reference exactly, and the port was verified against the
banked #330 per-track digits on all 215 val tracks.

The campaign's numbers, briefly (details in the c2a artifacts under the
campaign dir and decisions #328-#331): fine-tuning on the corpus's own grids
collected most of the stock-to-ceiling gap (offline downbeat F1 0.847 vs stock
0.697); windowing and causality cost nearly nothing; the fused causal grid
reads 0.8255 against the counting fallback's 0.4994; and decoded under the
frozen l9 decoder it buys **+0.0731 crispness@0.5 s -- 58.8 % of the measured
counting-to-expert gap -- while flicker improves** and macro/accuracy sit
within noise. Flips concentrate exactly where madmom truly slips, which is the
counting grid being silently wrong, not the tracker being unstable. The
remaining gap is mostly the madmom beat stream itself (the identical evidence
on expert beats scores 0.9398).

**The warm-up anchor story is subsumed, not repealed.** madmom's online warm-up
costs the first annotated beat, so the first beat the runtime sees is bar
position 1 on 147 of 215 val tracks; that measured first-beat prior is now the
HMM's start prior, so a track with no usable evidence converges to the anchor-1
counting grid the previous generation shipped -- the old fallback is the fixed
point of the new mechanism. The old pricing stands as history: counting cost
-0.1396 crispness@0.5 s against an expert grid, the anchor fix recovered
+0.0510, and the tracker now recovers most of the rest.
`tests/test_section_decoder_equivalence.py` still holds the *counting* path --
which every degradation route lands on -- to the grid that was priced.
...
still correct would trade it for a one-in-four guess. **The phase tracker's
births mirror the same three flavours**: a cold start takes the measured
first-beat prior, a beat-gap re-anchor takes a uniform prior (the true position
of the beat that ends a gap is measurably a coin toss) with the gap-ending beat
opening a bar exactly as the counting rule's re-anchor does, and a tracker
outage suspends the trellis so the count continues from the position it holds
-- the resume rebirth carries that position and lets evidence refuse it
forward-only, the decoder-rebirth pattern applied to phase. The two outage
births are unmeasured by construction (the campaign's offline harness has no
gaps and no sheds); both are pinned by unit tests and are one line to withdraw.
...
- **`lag_bars = 2` is the chosen point on a measured curve, not a default.** The
  ...
  option in every other respect too. **The 9-class sweep re-chose lag 2
  independently, on a differently-shaped curve**: its macro-vs-lag curve is
  monotone with no saturation at 2, and lag 3 sits exactly at the flicker
  ceiling — so the 14 s budget is what holds the line, the curve is banked for
  any future budget discussion, and the two generations agreeing on the point
  is a constraint fact rather than a coincidence (#302).
...
python auto_pilot label path/to/song.mp3 --seed            # ...starting from the published boundaries, so only names need changing
...
- **Beat This-small (vendored, MIT)** -- the bar tracker: a ~2M-parameter
  ...
  The minimal inference path is vendored in `lib/vendor/beat_this_infer.py`
  (license and attribution beside it) rather than added as a dependency -- the
  tree already carries torch, and the rewrite was verified bit-identical to the
  original model forward on the shipped checkpoint, with the frontend and
  resampler deltas measured and recorded in the artifact record. The checkpoint
  lives in the gitignored corpus models dir with its sha and geometry beside
  it, verified fatally at construction. The bar-phase HMM that consumes its
  evidence is `training/phase_tracking/`'s machinery, imported not copied.
...
- **One label column, and the fold is a view rather than a stage.** The table's three label columns collapsed to one `label` carrying the published section. The retired five-class fold survives only as a pure function applied at *evaluation* time, so no pipeline stage ever stores a folded label, and the banked historical record stays comparable without the data pipeline having to know the old space existed. Changing the model's class space still costs a re-join rather than a re-simulation, which is the property the three columns existed for.
- **Genre travels with the track, and a track that has none says so.** The published annotation's per-track genre is carried through the manifest and the clean manifest to a column on the training table, so a genre-conditioned selection or evaluation needs no second lookup; it is optional on a hand label and absent -- never blank-as-a-guess -- when the owner named none.
...
- **The intent alphabet and the label alphabet are different languages, so the confusion matrix is not square.** Rows are the intents the engine can commit, columns are the annotator's classes, cells are minutes of show. Flattening one alphabet into the other before looking at the matrix hides exactly the failures worth seeing. The mapping from intent to "the labels this intent is correct for" lives in a single dict at the top of the script and is the thing the owner iterates on. The matrix is rectangular in the other direction now -- nine classes, four intents -- because several classes deliberately share a look.
- **Dual granularity, one gated.** Everything is reported in the nine-class space (primary, and since the GATED_SPACE flip the one the benchmark gates) and again through the retired five-class fold (diagnostic), so figures cut before the cutover stay comparable with figures cut after it. The fold view is *label*-neutral by construction -- it composes exactly the retired chain, which is a testable claim about labels and is tested. The nine-class reading is what a successor model gets judged on; the folded one is scaffolding with a removal date.
...
- **"How often did it change" has two honest answers, and a model may only be scored against one of them.** The *intent stream* counts every committed `LightIntent` change - each one re-picks a lighting effect, so that is the show as the room experiences it and the owner's continuity number. The *class stream* maps into the label space first and then differences, so a move between two intents claiming the same class is not counted. With GROOVE and PEAK both retired no two intents claim the same class, so the two streams are now provably equal for the engine as well as for a model -- an equality the reports assert rather than assume, since printing one number twice would look like corroboration. Both are still reported, because the distinction returns the moment an engine-derived state does.
...
- **That checksum is over the file's bytes, so the checkout must not rewrite them.** Git on Windows defaults to `core.autocrlf=true` and materialises LF as CRLF, which changes the hash while changing nothing about the benchmark — the guard then passed in the worktree the file was written in and failed in every fresh clone, making its verdict a fact about the machine. `.gitattributes` pins the frozen artifacts to `eol=lf` so every checkout agrees with the writers, which already emit LF unconditionally. An older clone made before that pin keeps its CRLF copies until they are re-checked-out; rewriting them to LF is the remedy and leaves the index untouched. **Two things about that make the failure worse than it sounds, and both are now mechanised rather than written down.** `git status` reports the tree *clean* — git normalises on read, and the index still holds the stat data of the CRLF file git itself wrote — so the only symptom is a provenance check failing with a sha nobody can account for; the checks therefore say so themselves, by proving the LF-normalised bytes reproduce the pin before naming the cause, and a fast unit test fails on any pinned file whose working copy has drifted. And the obvious remedy is a no-op: `git checkout -- <path>` alone skips a file git believes matches, so the file has to be deleted first. A re-cut (`--cut`, `--write-baseline`) is never the remedy — the committed bytes were never wrong — and the two error paths that used to advise one now withhold that advice when the cause is line endings.
...
- **Every published section boundary is a real event now.** The boundary head used to *delete* the neighbourhood of any join where two published sections folded to the same class -- ambiguous by construction, so neither a positive nor a negative. With the folds retired there are no such joins, and that signal comes back: an `altintro` beat-in, for instance, is a taught boundary instead of a masked one, and roughly three quarters of the corpus had at least one. The deletion rule survives only for genuine *gaps* in the annotation, where there is no truth to teach either way, and a known boundary sitting inside a deleted neighbourhood still keeps its target.
...
- **The class stream is the only fair comparand, and for this model it is the only stream.** A label-space model cannot express a move between two intents claiming the same class, so quoting the engine's intent-stream flicker against it would once have overstated the model. Under identity claims the model's two streams are provably the same stream, which the report asserts rather than assumes -- printing one number twice would look like corroboration.
...
1. **Live bar tracking exists now, and it came through measurement rather than
   around it.** The counting fallback shipped first because every cheap phase
   estimator lost to it: a boundary-logit phase vote lost on all 120
   configurations (twice, on two designs), interval repair over-triggered 11x,
   and hand-designed percussive evidence (the #327 C1 arm) missed the
   interval-invisible slips entirely. What finally beat it (#322-#331) was
   in-genre training -- the survey's decisive number is that genre training
   data is worth more downbeat F1 than every architecture delta in the field
   combined -- via a Beat This-small fine-tuned on the corpus's own grids and
   run over trailing windows into the parked bar-phase HMM. The pre-registered
   flips gate failed as written and was shown mis-specified at the show layer
   (#330/#331: flips sit where madmom truly slips, corr(flips, crispness
   delta) ≈ 0), so adoption was ruled on decoded show numbers, not grid
   numbers. The counting rule survives verbatim as the degradation state.
   What remains open is the madmom beat stream itself: the identical evidence
   on expert beats scores ~0.11 higher, so the beat source is now the
   bottleneck, not the phase.

   **The offline machinery from the downbeat branch is still in `training/nn/`,
   parked**; the tracker that ships came from the c2a campaign instead, and
   `training/phase_tracking/` graduated from pricing harness to live code --
   `lib/engine/bar_phase.py` imports its trellis mathematics.
...
3. **Continuous bar tracking** -- done: the #332 adoption shipped the fused
   Beat This tracker (see item 1 above). The successor lever is the beat
   source itself, which now carries the remaining live-grid cost.
...
- **MusicAnalyser rolls its state** every 15 min to stop the rolling windows growing without bound. It is a statement about memory and says nothing about the music — and that distinction is load-bearing, because a continuous venue feed never trips the 0.3 s silence gate, so this horizon is the *only* boundary the engine would otherwise see. It used to go through the same reset that clears `is_playing`, which made the next buffer of uninterrupted audio announce a new song: the extractor ring, the student, the bar grid, the queued intents and the cold-start floor all restarted, four times an hour, for a track that never changed. The roll now keeps the show's own state; only the rolling windows are rebuilt. **It no longer resets the beat tracker either.** The old roll re-locked madmom's DBN mid-audio while the bar grid kept counting and nobody told it, so any beat the re-lock gained or lost rotated the grid permanently -- probed at five instants in one clip: the next beat landed 0.13-0.26 s late every time, and on two of the five the beat count moved (+2, +1). An intermittent rotation, invisible to every gate (no eval-set or corpus track reaches the horizon) and crossed four times an hour by a live set. The roll therefore keeps the whole beat stream -- the tracker's lock, the BPM window, and the beat count the OS2L position is derived from -- and re-arms only the song clock and the rolling windows. That is sound because madmom's online state is measured bounded rather than assumed: every array and container in the tracker's live object graph is size-identical after 11 minutes of locked audio (~1.6 MB, flat); what accumulates is scalars (the DBN's frame counter and last-beat instant, the adapter's hop count). The madmom contract suite pins that property, and a real song boundary still re-locks the tracker -- that reset is about the music, not memory.
...
- **The show's model artifacts are 1.3 GB and are not in git.** The encoder, the student's graph, the priors and the bar tracker's checkpoint (with its sha/geometry record beside it, under the generation dir's `bar_tracker/`) live in the gitignored corpus directory. `lib/section_chain.artifacts_present()` is the one-line question for the section chain and `bar_tracker_present()` the separate one for the tracker — the tracker is optional on top of the chain, absent-is-counting, present-must-verify; without the chain the app logs a warning at startup and runs the degradation state, and the `nn_artifacts` test fixture skips. The benchmark treats the model as a third input beside the audio and the labels: `run_eval_set.missing_model` refuses before anything is simulated, because a degraded run moves every checksum and zeroes every score, and re-cutting a baseline from *that* would write ten tracks of dark show over the benchmark.
- **Two checkouts are reachable from one `sys.path`, and whichever imports first wins silently.** Several offline scripts put the phase-B worktree on the path because the training code for the shipped generations exists nowhere else; `training.nn` then resolves there or here depending on path order and on what was imported before. Nothing in a run's output says which one ran, so a measurement can be of a codebase nobody named — it was hit for real as an `AttributeError` on a knob one checkout has and the other does not, which is the *lucky* version of this failure. Every script that names a second checkout now declares which one its `training.nn` must come from and is held to it at import (`training/module_source.py`), so the answer cannot be import order. Which checkout each one wants is a real distinction, not a formality: the gates need phase-B's `ceiling`/`paired`, and the rebirth case replays phase-B *artifacts* through this repo's decoder.
- **A probe rig that cannot find the shipped generation must refuse, not degrade.** The campaign probe rigs build a shadow corpus mirroring the shipped generation and hand it to the real sim through `$RAVEFORM_DATA_DIR`. The generation dir name used to be typed into the rig, so the next ship left it pointing at the previous generation, `section_chain` found no artifacts under the name it resolves, and the sim ran the **degradation state** — producing a number that reads exactly like a real probe. The rig now takes both the generation and the model version off `lib/section_chain` (the one place that knows what the show resolves) and refuses loudly when the source generation or the built shadow is not a chain the show can resolve — the same contract `run_eval_set.missing_model` applies to the benchmark, for the same reason.
...
- **Three caches beside the audio now**: `simulate file` writes `<song>.<samplerate>.npy` (the decode), `<song>.<decoder>.mertcells.npz` (the extractor's cells) and `<song>.<decoder>.bartracker.npz` (the tracker's chunks). All are gitignored, all are keyed on the audio's size and mtime, and deleting one forces that stage to re-run. A warm run replays both model stages or neither — one decision, so a half-warm run cannot silently mix a replayed stage with a recomputed one. The ten eval-set tracks keep their derived files on purpose (see The benchmark); everything else in the corpus is cleaned up by the batch that created it. A sidecar recorded under one decode path is never served to the other — that is in the filename and in the key, because librosa and ffmpeg move 13.2 % of near-boundary decisions.
- **The bar tracker fails toward counting, never toward a shed.** Missing artifacts build the chain without it; artifacts that fail sha or geometry verification refuse construction fatally; a forward that starts failing mid-show marks the tracker dead, and the fusion freezes its offset so the grid continues as the counting rule from the position it holds. A watchdog shed suspends it the same way — the section stage's degradation contract is not made hostage to the optional stage. In the threaded paths a vote can arrive a beat later than the fast sim places it (real hand-off latency); that jitter is the same class as the posterior stream's and is only observable where real threads run.
