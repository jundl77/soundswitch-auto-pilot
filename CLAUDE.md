...
- **Excluding the blackout changed no score, and the thing that did change one was the block before it.** Worth writing down because it was diagnosed the expensive way: on the one eval-set track whose numbers moved, the quiet floor sits past the last label and covers no beat at all, so its exclusion is arithmetically a no-op. What moved the score is that the preceding block stopped being the *final* block. `realign_intents` ends a final block at the report duration de-shifted by its own delay, and a non-final one at the next block's start -- so appending any block at all re-ends its predecessor. The pre-A2 number was the artifact of the two: being last, that block was cut a full look-ahead early and simply had no opinion on the ~11 s of labelled beats it had actually committed to. The stop is a real instant about real audio, so ending the block there is the truthful reading and the slightly worse score is the honest one. **A block's end is inherited from its neighbour rather than measured**, which is fine while every block is a classifier commit and is the first thing to suspect the moment they are not; making each block own its end is a real change to consider, and it would move every track, so it belongs to its own baseline rather than to a stop-behaviour commit.
