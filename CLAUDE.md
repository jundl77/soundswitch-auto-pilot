...
- **Recovery is selective, not blanket.** A plain downloader re-run skips every recorded failure and a `--retry-failed` re-run re-polls genuinely dead videos forever. Retry passes therefore name the reasons worth re-attempting (refusals, timeouts, empty/missing output) and leave removed and private videos alone.
