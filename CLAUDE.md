...
The model is the l9b campaign's arm-H w128 seed-1234 student (#341/#343) —
the l9 recipe retrained after the owner's 33-track audit put his hand relabels
over the published classes (#338/#339: boundaries gold, drop over-called on
mellow material, buildup badly carved) — selected on decoded val output exactly
as its predecessor was (#301/#302 — frame-macro seed ordering flips under the
decoder, which is why selection happens there). Its priors are the refit over
those corrected labels, and the decoder config is that campaign's sweep pick —
the per-class floor axis opened, buildup down to one bar — committed at
...
destroys.** `models/l9b/` in the corpus is the generation dir — the shared F3
affine (byte-identical copy of l9's, itself phase_b's; every generation so far
trained on the same feature stream), the refit priors, the campaign's decoder
config, and the verified export under `ng_H_w128_s1234/` with its
...
retired generations stay on disk untouched — `models/l9/` in full, one constant
away as the rollback, and `models/phase_b*` still readable by the
rebirth/phase-tracking gates that pin decoder *mechanisms* against banked
5-class cases. The campaign dir (`models/l9b_campaign/`) is the training
record — the decoded verdict board, the probe reads, the per-arm results and
the label-target artifacts — and archives to D: at the generation's DONE
boundary (#298), which is exactly why the shipped set lives outside it.

**The campaign's own gate split, and the ship is the passing half.** The owner's
two-part gate (#341-#343) asked for the decoded-val board not to regress AND for
the two named probe tracks' buildup arcs to commit. The board half passed with
margin — arm H reads better than the l9 student on every gated axis, flicker
within the ceiling (`models/l9b_campaign/NG_DECODED.md`) — but seven
modified runs across five mechanisms all failed the probe half, so #343 closed
it as a **measured wall**: the probes' buildup shapes are acoustically atypical
and two exemplars among a thousand tracks cannot re-shape the class, whatever
the sampler does. The known limitation ships stated rather than smoothed over:
buildup on slow climbs and micro-ramps still reads as its neighbours, drops
still land on time, and the path forward is more owner labels — the campaign's
`LABEL_TARGETS.md` names exactly which tracks to label next, and the two probes
stay the regression reads for every future generation.
