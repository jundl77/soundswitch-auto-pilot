...
- **Dual granularity while the migration settles.** Everything is reported in the nine-class space (primary) and again through the retired five-class fold (diagnostic), so figures cut before the cutover stay comparable with figures cut after it. The fold view is score-neutral by construction -- it composes exactly the retired chain -- which is what lets the committed eval-set baseline keep its meaning across the change. The nine-class reading is what a successor model gets judged on; the folded one is scaffolding with a removal date.
