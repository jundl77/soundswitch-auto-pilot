...
The model is the l9 campaign's w128 seed-1234 student (selected on decoded val
output, #301/#302 — frame-macro seed ordering flips under the decoder, which is
why selection happens there), its priors are the 9-class refit, and the decoder
config is the 9-space sweep's pick, committed at `training/nn/decoder_config.json`
with the class space it was swept in recorded inside it. `build_section_chain`
now enforces **space equality, fatally**: the graph's declared label axis, the
...
benchmark's gated space flipped to the raw nine in the same change, and its
...
destroys.** `models/l9/` in the corpus is the generation dir — the shared F3
affine (byte-identical copy of phase_b's; the campaign trained on the same
feature stream), the refit priors, and the verified export under
`l9_w128_s1234/` with its sha/geometry/parity record beside it. The retired
5-class set stays on disk untouched under `models/phase_b*`, still readable by
the rebirth/phase-tracking gates that pin decoder *mechanisms* against banked
5-class cases. The campaign dir (`models/l9_campaign/`) is the training record
and archives to D: at the generation's DONE boundary (#298), which is exactly
why the shipped set lives outside it.
