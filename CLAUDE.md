...
- **Dual granularity while the migration settles.** Everything is reported in the nine-class space (primary) and again through the retired five-class fold (diagnostic), so figures cut before the cutover stay comparable with figures cut after it. The fold view is *label*-neutral by construction -- it composes exactly the retired chain, which is a testable claim about labels and is tested. It is emphatically not a claim that scores are unchanged: removing PEAK moved the committed intent stream, so every eval-set checksum and every intent-side score moves with it and the baseline is re-cut once. The nine-class reading is what a successor model gets judged on; the folded one is scaffolding with a removal date.
