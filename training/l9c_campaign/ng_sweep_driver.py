"""The per-arm decoder sweep for the l9c campaign (#346).

l9b's sweep driver with two disclosed changes: the instrument imports from the
MAIN repo (the #345 buildup_drop_bonus knob lives there; nn/* is
committed-identical to the label9 tree, and the verdict's banked-digit anchors
prove the instrument did not move), and the #345 transition axis joins the
grid as an extra stage beside floor_bars and outro_escape -- so the axis is
searched, ablated and joint-refined exactly like every other knob, and the
chosen config records whatever value won under the unchanged selection rule
(max two-seed-mean val macro subject to the flicker ceiling and lag <= 2).
"""
import argparse
import ctypes
import dataclasses
...
# #345/#346 pre-registered transition axis: log-domain buildup->drop entry
# preference.  0.0 is today's decoder; 0.4/0.7/1.1/1.6 are ~ln(1.5/2/3/5).
BUILDUP_DROP_BONUSES = (0.0, 0.4, 0.7, 1.1, 1.6)
...
def main() -> int:
    ...
    parser.add_argument("--arm", default=None,
                        help="arm label, e.g. N (runs default to "
                             "ng_<ARM>_w128_s1234,s1235)")
    ...
    say(f"buildup_drop_bonus axis (#345): {bonuses}")
    ...
    result = run_sweep(caches, priors, flicker_ceiling=ceiling, log=log,
                       ...
                       extra_stage_axes={"floor_bars": floors,
                                         ...
                                         "buildup_drop_bonus": bonuses})
    elapsed = time.perf_counter() - started

    payload = {
        "generated_at": datetime.datetime.now(datetime.timezone.utc).isoformat(),
        "label": f"l9c campaign decoder sweep, arm {label}"
        ...
        "buildup_drop_bonus_axis": list(bonuses),
    ...
    config = {
        ...
        "notes": [
            ...
            "buildup_drop_bonus is the #345/#346 transition axis (log-domain "
            "buildup->drop entry preference); 0.0 is the pre-l9c decoder "
            "exactly",
