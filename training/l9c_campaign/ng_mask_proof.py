"""L9C mask mechanical proof: the masked spans contribute ZERO label loss
terms against the CURRENT merged annotation view, and nothing else moves.

l9b's ARM H-MK proof (#343) pointed at the l9c overlay -- the hand-excluded
cut -- and the l9c campaign dir.  Runs BEFORE any training, CPU only, through
the trainer's own modules (``mask.apply_supervision_mask`` +
``dataset.track_targets`` under the l9 class space, frames counted exactly as
``FullTrackDataset`` counts them).  The merged view now carries the owner's
~28 hand relabels; the loader's own hand-label hard-refusal runs as part of
``apply_supervision_mask``, so this proof also demonstrates the l9c overlay
clears it.  Checks, per overlay span:
...
def main() -> int:
    ...
    proof = {
        ...
        "applied_stats": stats,
        "hand_excluded": overlay.get("hand_excluded"),
