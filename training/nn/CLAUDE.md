...
>
> The pipeline has since moved a **second** time, taking master's post-review simplification, and that move does not add anything to date. It moved report *content* — a bounded dispatch timing log — while leaving every beat and every intent change on the eval set bit-identical, so the beat instants this chain lifted from the cached sim reports are the instants it always had. What did change is that a re-run would now regenerate those inputs under a third pipeline sha, which is precisely the second read #84 exists to prevent. The artifacts on disk are the record.
...
| `models/downbeat_<gen>/downbeat_alignment_val.json` | the corpus aubio-vs-expert grid analysis: per-track offset and jitter, and every annotated downbeat labelled with why it is or is not reachable |
