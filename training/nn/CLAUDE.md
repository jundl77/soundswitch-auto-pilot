...
A **second chain is under construction beside it**: a downbeat head and a bar-phase decoder, because the section decoder commits at bar rate and live audio arrives with beats but no bar. It shares the sidecars, the splits and the window machinery, and it is the deployment prerequisite the v1 verdict named. Spec: `docs/superpowers/specs/2026-07-27-live-downbeat-tracking-design.md`. What is here is the machinery; its v1 scoring is not, for the reason in the note below.
...
| `downbeat_decoder.py` | the bar-phase HMM: a cyclic position within the bar, committed at a fixed lag and never revised, coasting through the beat stream's dropouts and reporting how sure it is; owns the candidate grid and the activation-aggregation window, because in the runtime putting an activation onto a beat stream *is* decode-path work; pure numpy, because the runtime will import this one |
