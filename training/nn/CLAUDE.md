...
**It stopped being a deployment prerequisite and became parked training work.** The runtime integration shipped without it: live bars are four beats counted off the beat stream from one beat in (rulings #157/#158/#189), a fallback chosen because a boundary-logit phase vote lost to plain counting on every configuration tried -- and then lost again, monotonically in the weight given to the head, on a continuous fixed-lag filter with tie-free reads. That is the third independent falsification; the head is not phase evidence. Nothing in `lib/` imports anything in this chain. The re-scoped successor is continuous bar tracking, and the price the show pays until it exists is written down in the root `CLAUDE.md`.
