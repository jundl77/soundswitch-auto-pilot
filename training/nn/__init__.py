...
``select_eval_set``) and the corpus-acquisition scripts one level below
(``training/raveform/``: ``build_clean_manifest``, ``raveform_manifest``,
``raveform_fetch_annotations``) are plain modules rather than an installed
package, so importing them requires their directories on ``sys.path``.  Doing
it here, once, keeps every module in this package -- and every test that
imports one -- from repeating the incantation, and guarantees the label
vocabulary, the mel geometry and the artist parser are the *same* definitions
the corpus was built with rather than copies that can drift.
"""
...
_RAVEFORM_DIR = str(Path(__file__).resolve().parents[1] / "raveform")
for _dir in (_TRAINING_DIR, _RAVEFORM_DIR):
