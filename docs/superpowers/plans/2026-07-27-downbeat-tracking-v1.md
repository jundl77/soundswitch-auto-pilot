...
**Amendment during Task 3 (coordinator decision, recorded in the spec):** the phase state space became *half-beat-resolvable* — a `subdivision` decoder parameter that admits the midpoint of each consecutive beat pair as a candidate instant, doubling the cycle to eight positions, so a stream sitting half a beat off the bar is a state the decoder can hold rather than an error it rounds away. A separately-togglable continuous instant refinement is also shipped, off by default, so a gain can be attributed to candidate-grid resolution, phase model or de-quantisation rather than blended.

**Superseded (owner decisions #81/#133):** this plan's acceptance gate below and the verdict it produced both bound to the aubio beat stream, which madmom has since replaced. The aubio-condition results are removed from the repo rather than kept as a dated record; the 0.85 gate is retired and the recommended replacement is F1 ≥ 0.55 at a median of two phase flips per track or fewer, measured on madmom's stream. Successor numbers: `docs/migration-evidence.md`. On that stream the best candidate grid is subdivision 1, so the half-beat state space is droppable in the next iteration.
